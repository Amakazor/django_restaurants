class StylizedSelect {
  ...
  onOpenChange = () => {
    if (this.isOpen) {
      this.stylizedSelectElement.classList.add(
        "stylized_select_element--active"
      );
      this.optionDictionary.forEach(
        (option) => (option.stylizedOption.tabIndex = 0)
      );
    } else {
      this.stylizedSelectElement.classList.remove(
        "stylized_select_element--active"
      );
      this.optionDictionary.forEach(
        (option) => (option.stylizedOption.tabIndex = -1)
      );
    }
  };
  ...
  getOptionElement = (stylizedOptionElement) => {
    return this.optionDictionary.find(
      (option) => option.stylizedOption === stylizedOptionElement
    ).option;
  };
  ...
  constructor(element) {
    ...
    this.stylizedElement.classList.add(
      "stylized_select--" +
        (this.element.dataset.variant === "top" ? "top" : "bottom")
    );
    ...
    Object.values(this.options).forEach((option) => {
      ...
      this.optionDictionary.push({
        option: option,
        stylizedOption: stylizedOptionElement,
      });
      stylizedSelectOptionlist.appendChild(stylizedOptionElement);
    });

    this.element.parentNode.insertBefore(
      this.stylizedElement,
...
document.addEventListener("DOMContentLoaded", () =>
  document
    .querySelectorAll(".stylizedSelect")
    .forEach((element) => new StylizedSelect(element))
);
